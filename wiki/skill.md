...
dworkspace can write an instruction bundle for itself: a small zip of Markdown that
...
It contains four Markdown documents in a folder called `dworkspace/` — nothing
...
saying "connect to your Dworkspace instance" could live in any repository, and it would
...
The browser saves `dworkspace-skill.zip`. Unzip it and you get a `dworkspace/` folder.
...
curl -H "Authorization: Bearer <token>" \
     "https://dworkspace.example.com/api/skill?workspace=<workspace-id>" \
     -o dworkspace-skill.zip
```
...
Opens with a small YAML header (`name: dworkspace` and a description telling an
...
most often against a dworkspace collection:
...
**Claude Code** — put the `dworkspace/` folder in either place:

```
~/.claude/skills/dworkspace/       every project
.claude/skills/dworkspace/         this repository only
```

Then invoke it with `/dworkspace`, or just ask to work with Dworkspace.
...
It closes with one line: *If it is not recorded in dworkspace, it did not happen.*
...
playbook itself. Unpack the new bundle over the old `dworkspace/` folder: it is the
