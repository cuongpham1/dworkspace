...
const backend = process.env.DWORKSPACE_PROXY ?? 'http://localhost:8420';
...
// exists for. Hand-syncing two numbers was never going to hold; DWORKSPACE_VERSION
// comes from the build (see Dockerfile's ldflags) and 'dev' locally, where a
// mismatch is meaningless anyway.
const version = process.env.DWORKSPACE_VERSION ?? 'dev';

export default defineConfig({
  define: {
    __DWORKSPACE_VERSION__: JSON.stringify(version),
